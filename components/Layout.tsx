...
export const Layout: React.FC<LayoutProps> = ({ children, onPrint, hasResult }) => {
  return (
    <div className="min-h-screen min-w-0 overflow-x-clip bg-slate-50 flex flex-col text-slate-800 antialiased selection:bg-blue-100 selection:text-blue-900">
      {/* Top Banner & Header */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between min-h-16 py-2 gap-2">
            <div className="flex min-w-0 items-center gap-2 sm:gap-3">
              <div className="shrink-0 bg-blue-600 text-white p-2 rounded-lg flex items-center justify-center shadow-inner">
                <MapPin className="w-5 h-5" />
              </div>
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="truncate text-[10px] sm:text-xs font-semibold tracking-wider text-blue-400 uppercase">Hokkaido AI Subsidy Agent</span>
                ...
                <h1 className="truncate text-sm sm:text-lg font-bold text-white tracking-tight leading-tight">
                  北海道補助金AI診断
                </h1>
              </div>
            </div>
            ...
            <div className="shrink-0 flex items-center gap-2 sm:gap-3">
              ...
              {hasResult && onPrint && (
                <button
                  type="button"
                  onClick={onPrint}
                  className="inline-flex min-h-11 items-center gap-1.5 px-3 py-2 text-xs font-semibold text-slate-200 bg-slate-800 hover:bg-slate-700 rounded-lg border border-slate-700 transition-colors shadow-sm"
                  title="A4印刷・PDF出力"
                >
      ...
      <main className="flex-1 min-w-0 w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8">
        {children}
      </main>
